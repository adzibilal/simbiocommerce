...
const SingleItem = ({ item }) => {
  ...
  return (
    <div className="flex items-center gap-4">
      <div className="flex items-center justify-center rounded-[10px] bg-gray-3 w-[72px] h-[72px] shrink-0 overflow-hidden">
        <Image src={item.imageUrl || "/images/products/default-product.png"} alt={item.name} width={72} height={72} className="w-full h-full object-cover" />
      </div>

      <div className="flex-1 min-w-0">
        <h3 className="font-medium text-dark text-sm truncate mb-1">{item.name}</h3>
        <p className="text-custom-sm text-dark-4 mb-2">{formatCurrency(item.price)}</p>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="w-7 h-7 flex items-center justify-center rounded-md border border-gray-3 text-dark hover:bg-gray-2 text-base font-medium"
          >
            −
          </button>
          ...
          <button
            ...
            className="w-7 h-7 flex items-center justify-center rounded-md border border-gray-3 text-dark hover:bg-gray-2 text-base font-medium disabled:opacity-40 disabled:cursor-not-allowed"
          >
            +
          </button>
        </div>
