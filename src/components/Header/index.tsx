...
const Header = ({ storeInfo }: { storeInfo: StoreInfoData | null }) => {
  ...
  const { localIds, products: rvProducts } = useRecentlyViewed();
        ...
        >
          ...
          <div className="xl:w-auto flex-col sm:flex-row w-full flex sm:justify-between sm:items-center gap-5 sm:gap-10">
            <Link className="flex-shrink-0" href="/">
              <Image
                src={storeInfo?.logoUrl || "/images/logo/logo.svg"}
            ...
            <div className="max-w-[475px] w-full">
              <form onSubmit={handleSearchSubmit}>
                <div ref={searchRef} className="relative">
                  <div className="flex items-center bg-gray-1 border border-gray-3 rounded-[5px]">
                    <input
                      ...
                      autoComplete="off"
                      className="w-full bg-transparent py-2.5 pl-4 pr-10 outline-none text-sm"
                      onFocus={() => searchQuery.trim() && setShowSuggestions(true)}
                    />
                    <button
                      type="submit"
                      aria-label="Search"
                      className="flex items-center justify-center px-3 text-body hover:text-blue ease-in duration-200"
                    >
                  ...
                  {showSuggestions && suggestions.length > 0 && (
                    <div className="absolute left-0 right-0 top-full mt-1 bg-white rounded-xl shadow-xl border border-gray-2 z-50 overflow-hidden">
                      {suggestions.map((p) => (
                        <button
                          key={p.id}
                          type="button"
                          onMouseDown={() => handleSuggestionClick(p.slug)}
                          className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-gray-1 transition-colors text-left"
                        >
                          <div className="w-10 h-10 rounded-lg overflow-hidden bg-gray-2 shrink-0">
                            {p.imageUrl && (
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-body">{formatCurrency(p.price)}</p>
                          </div>
                        </button>
                      ))}
                      <button
                        type="submit"
                        onMouseDown={handleSearchSubmit as any}
                        className="w-full px-4 py-2.5 text-sm text-blue hover:bg-blue/5 transition-colors text-left border-t border-gray-2"
                      >
              ...
              <div className="flex items-center gap-5">
                ...
                {session?.user ? (
                  <div className="relative group flex items-center gap-2.5 cursor-pointer">
                    ...
                    {/* Dropdown */}
                    <div className="absolute top-full right-0 mt-2 w-44 bg-white rounded-lg shadow-lg border border-gray-3 invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all duration-200 z-50">
                      <Link href="/my-account" className="block px-4 py-2.5 text-sm text-dark hover:bg-gray-1 rounded-t-lg">My Account</Link>
                      <Link href="/my-account" className="block px-4 py-2.5 text-sm text-dark hover:bg-gray-1">My Orders</Link>
                      <button
                        onClick={() => signOut({ callbackUrl: "/signin" })}
                        className="w-full text-left px-4 py-2.5 text-sm text-red hover:bg-gray-1 rounded-b-lg"
                      >
                        Sign Out
                      </button>
                    </div>
            ...
            <div className="hidden xl:block">
              <ul className="flex items-center gap-5.5">
                <li className="py-4 relative">
                  ...
                  {/* Dropdown */}
                  {rvOpen && (
                    <div className="absolute right-0 top-full mt-1 w-72 bg-white rounded-xl shadow-xl border border-gray-100 z-50 p-3">
                      <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 px-1">Recently Viewed</p>
                      {rvProducts.length === 0 && localIds.length === 0 ? (
                        <p className="text-sm text-gray-400 px-1 py-2">No recently viewed products.</p>
                      ) : (
                        <ul className="space-y-2 max-h-72 overflow-y-auto">
                          {rvProducts.map((p: any) => (
                            <li key={p.id}>
                              <Link href={`/shop-details/${p.slug}`} onClick={() => setRvOpen(false)} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 transition-colors">
                                <div className="relative w-12 h-12 rounded-lg overflow-hidden bg-gray-100 shrink-0">
                                  {p.imageUrl && <img src={p.imageUrl} alt={p.name} className="w-full h-full object-cover" />}
                                </div>
                                <div className="flex-1 min-w-0">
                                  <p className="text-sm font-medium text-dark truncate">{p.name}</p>
                                  <p className="text-xs text-gray-500">{p.price?.toLocaleString("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 })}</p>
                                </div>
                              </Link>
                            </li>
                          ))}
                          {rvProducts.length === 0 && localIds.length > 0 && (
                            <li className="text-sm text-gray-400 px-1 py-2">{localIds.length} product(s) viewed — sign in to see details.</li>
                          )}
                        </ul>
                      )}
                    </div>
                ...
                <li className="py-4">
                  ...
                  >
                    ...
                    Wishlist
                    {wishlistCount > 0 && (
                      <span className="ml-1 bg-blue text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">{wishlistCount}</span>
                    )}
                  </Link>
                </li>
              </ul>
